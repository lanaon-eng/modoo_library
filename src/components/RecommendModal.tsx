...
export function RecommendModal({ open, book, followingList, onSend, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:items-center">
      <div className="mx-auto w-full max-w-md animate-slide-up rounded-t-3xl bg-white p-5 shadow-2xl dark:bg-ink-900 sm:rounded-3xl">
        {/* Header */}
        ...
        {/* Book preview */}
        <div className="mt-4 flex items-center gap-3 rounded-xl bg-ink-50 p-3 dark:bg-ink-800/50">
          {book.thumbnail ? (
            ...
            <div className="flex h-14 w-10 items-center justify-center rounded bg-ink-200 dark:bg-ink-700">
              <span className="text-[8px] text-ink-400">표지</span>
            </div>
          )}
          <div className="min-w-0">
            <p className="truncate text-[13px] font-bold">{book.title}</p>
            <p className="truncate text-[11px] text-ink-400">{book.authors.join(', ')}</p>
          </div>
        </div>
        ...
        {success ? (
          <div className="flex flex-col items-center py-8">
            ...
            <button
              onClick={onClose}
              className="mt-5 rounded-xl bg-brand-500 px-6 py-2.5 text-[13px] font-bold text-white transition-colors hover:bg-brand-600"
            >
              확인
            </button>
          </div>
          ...
          <>
            {/* Friend selection */}
            <p className="mt-4 text-[12px] font-semibold text-ink-500 dark:text-ink-400">추천할 친구</p>
            {followingList.length === 0 ? (
              <div className="mt-2 rounded-xl border border-dashed border-ink-200 py-6 text-center dark:border-ink-700">
                <p className="text-[12px] text-ink-400">팔로우한 친구가 없어요</p>
                <p className="mt-1 text-[11px] text-ink-400">피드에서 사용자를 팔로우한 후 추천할 수 있어요</p>
              </div>
            ) : (
            ...
            <textarea
              ...
              className="mt-3 h-16 w-full resize-none rounded-xl border border-ink-200 bg-ink-50 px-3 py-2.5 text-[13px] font-medium text-ink-800 placeholder:text-ink-400 focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100 dark:placeholder:text-ink-600"
            />
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-brand-500 py-3 text-[14px] font-bold text-white transition-all hover:bg-brand-600 active:scale-95 disabled:opacity-50"
            >
