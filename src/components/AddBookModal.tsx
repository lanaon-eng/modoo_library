...
export function AddBookModal({ open, onClose, onAdd, existingIds }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      {/* Sheet */}
      <div className="relative w-full max-w-md animate-slide-up rounded-t-3xl bg-white shadow-2xl dark:bg-ink-900 sm:animate-scale-in sm:rounded-3xl">
        {/* Grab handle */}
        <div className="flex justify-center pt-2.5 sm:hidden">
          <div className="h-1 w-9 rounded-full bg-ink-200 dark:bg-ink-700" />
        </div>
        ...
        <div className="flex items-center justify-between px-5 pt-4">
          ...
          <button
            onClick={onClose}
            aria-label="닫기"
            className="flex h-9 w-9 items-center justify-center rounded-full text-ink-400 transition-colors hover:bg-ink-100 hover:text-ink-900 dark:hover:bg-ink-800 dark:hover:text-ink-100"
          >
            <X size={20} />
          </button>
        </div>
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-ink-200 bg-ink-50 px-3.5 py-2.5 transition-colors focus-within:border-brand-400 focus-within:bg-white dark:border-ink-700 dark:bg-ink-800 dark:focus-within:bg-ink-900">
            ...
            {query && (
              <button
                onClick={() => setQuery('')}
                aria-label="검색어 지우기"
                className="flex h-6 w-6 items-center justify-center rounded-full text-ink-400 hover:bg-ink-200 dark:hover:bg-ink-700"
              >
                <X size={14} />
              </button>
            )}
        ...
        <div className="max-h-[52vh] overflow-y-auto px-5 py-4 no-scrollbar">
          ...
          {!loading && !searched && (
            <div className="flex flex-col items-center justify-center gap-2 py-12 text-ink-300 dark:text-ink-600">
            ...
            <ul className="space-y-2.5">
              {results.map((book) => {
                ...
                return (
                  <li
                    key={book.id}
                    className="flex items-center gap-3 rounded-2xl border border-ink-200/70 bg-white p-2.5 transition-colors dark:border-ink-800/70 dark:bg-ink-800/50"
                  >
                    <BookThumb book={book} />
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 truncate text-[11.5px] font-medium text-ink-500 dark:text-ink-400">
                    ...
                    {justAdded ? (
                      ...
                      <span className="rounded-full bg-ink-100 px-3 py-1.5 text-[11px] font-bold text-ink-400 dark:bg-ink-700 dark:text-ink-400">
                        보유중
                      </span>
                    ) : (
                      <button
                        onClick={() => handleAdd(book)}
                        className="shrink-0 rounded-full bg-brand-500 px-3.5 py-1.5 text-[12px] font-bold text-white transition-all hover:scale-105 hover:bg-brand-600 active:scale-95"
                      >
                        담기
                      </button>
                    )}
