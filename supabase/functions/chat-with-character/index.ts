...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { bookTitle, bookContents, role, messages } = body;

    if (!bookTitle || !role || !messages || messages.length === 0) {
      return new Response(
    ...
    const systemPrompt = `당신은 「${bookTitle}」이라는 책 속의 ${roleInfo.label}입니다.
${roleInfo.desc}
